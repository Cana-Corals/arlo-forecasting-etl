...
.dm-sub    { font-size:33px; color:var(--muted); }
...
.dm-kpi-sub { font-size:30px; color:var(--muted2); margin-top:5px; }

.dm-section { padding:16px 20px 0; }
.dm-section-ttl { font-size:27px; font-weight:600; letter-spacing:.16em; text-transform:uppercase; color:var(--muted); margin-bottom:8px; }
</style>
""", unsafe_allow_html=True)
...
with _bb1:
    ...
    lt_lay["title"] = dict(text="Lead Time Distribution",
                           font=dict(size=33, color="rgba(245,245,240,0.7)"),
