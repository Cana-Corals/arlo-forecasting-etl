...
.mi-sub    { font-size:16px; color:var(--muted); }
...
.fi-name { font-size:14px; color:rgba(245,245,240,.75); min-width:220px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
DARK = dict(
    ...
    font=dict(color="rgba(245,245,240,0.7)", family="Inter, system-ui", size=14),
...
with tab_actual:
    ...
    for i, target in enumerate(["revenue", "occupancy", "adr", "revpar"]):
        ...
        _dark(fig)
        fig.update_layout(
            title=dict(text=title_txt, font=dict(size=14, color="rgba(245,245,240,.8)")),
    ...
    with fi_col2:
        ...
        fig_fi.update_layout(
            title=dict(
                ...
                font=dict(size=14, color="rgba(245,245,240,.8)")
            ),
