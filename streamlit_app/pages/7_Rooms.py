...
# ── OOO Trend by Room Type ────────────────────────────────────────────────────
st.markdown('<div class="rm-section"><div class="rm-section-ttl">Out of Order Rooms — Trend by Room Type</div></div>',
            unsafe_allow_html=True)

_ooo_all_rts = sorted(rt_f["room_type"].unique())
_ooo_sel_col, _ooo_view_col, _ = st.columns([3, 2, 3])
with _ooo_sel_col:
    _ooo_sel_rts = st.multiselect(
        "Room types", _ooo_all_rts, default=_ooo_all_rts,
        key="rm_ooo_rts", label_visibility="collapsed",
    )
with _ooo_view_col:
    _ooo_view = st.segmented_control(
        "View", ["Stacked", "Per Room"], default="Stacked",
        key="rm_ooo_view", label_visibility="collapsed",
    )

_ooo_sel_rts = _ooo_sel_rts or _ooo_all_rts

_ooo_fkey = "D" if n_days_period <= 14 else ("W" if n_days_period <= 60 else "M")
_ooo_dfmt = "%b %d" if n_days_period <= 60 else ("%b '%y" if n_days_period > 365 else "%b")

rt_f_ooo = rt_f.copy()
rt_f_ooo["p"] = rt_f_ooo["business_date"].dt.to_period(_ooo_fkey)
_ooo_agg = (
    rt_f_ooo[rt_f_ooo["room_type"].isin(_ooo_sel_rts)]
    .groupby(["room_type", "p"])["ooo_rooms"]
    .sum()
    .reset_index()
)
_ooo_periods = sorted(_ooo_agg["p"].unique())
_ooo_labels  = []
for _p in _ooo_periods:
    try:    _ooo_labels.append(_p.start_time.strftime(_ooo_dfmt))
    except: _ooo_labels.append(str(_p))

_OOO_COLORS = [
    "#e8854a","#3ecf8e","#4a6fa5","#d4903a","#e05252",
    "#9b59b6","#1abc9c","#f39c12","#2980b9","#c0392b","#27ae60","#8e44ad",
]
_ooo_rt_color = {rt: _OOO_COLORS[i % len(_OOO_COLORS)] for i, rt in enumerate(_ooo_all_rts)}

fig_ooo = go.Figure()
for _rt in _ooo_sel_rts:
    _rv = _ooo_agg[_ooo_agg["room_type"] == _rt]
    _yv = [float(_rv[_rv["p"] == _p]["ooo_rooms"].values[0]) if len(_rv[_rv["p"] == _p]) else 0
           for _p in _ooo_periods]
    if _ooo_view == "Stacked":
        fig_ooo.add_trace(go.Bar(
            x=_ooo_labels, y=_yv, name=_rt,
            marker_color=_ooo_rt_color[_rt],
            hovertemplate=f"{_rt}: %{{y:.0f}} rooms<extra></extra>",
        ))
    else:
        fig_ooo.add_trace(go.Scatter(
            x=_ooo_labels, y=_yv, name=_rt,
            line=dict(color=_ooo_rt_color[_rt], width=1.8),
            hovertemplate=f"{_rt}: %{{y:.0f}} rooms<extra></extra>",
        ))

_ooo_lay = dict(
    paper_bgcolor="#1a1a1a", plot_bgcolor="#1a1a1a",
    font=dict(family="Inter", color="rgba(245,245,240,0.5)", size=10),
    margin=dict(l=4, r=4, t=28, b=4), height=300,
    legend=dict(bgcolor="rgba(0,0,0,0)", borderwidth=0,
                orientation="h", x=0, y=1.14,
                font=dict(size=10, color="rgba(245,245,240,0.6)")),
    xaxis=dict(gridcolor="rgba(255,255,255,0.05)", tickfont=dict(size=9), tickangle=-30),
    yaxis=dict(gridcolor="rgba(255,255,255,0.05)", tickfont=dict(size=9),
               zeroline=False, title="OOO Rooms"),
    hovermode="x unified",
)
if _ooo_view == "Stacked":
